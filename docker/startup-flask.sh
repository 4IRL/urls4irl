...
flask db upgrade || {
  echo "FATAL: flask db upgrade failed" >&2
  exit 1
}
flask utils verify-tables || {
  echo "FATAL: flask utils verify-tables failed" >&2
  exit 1
}
flask shorturls add || {
  echo "FATAL: flask shorturls add failed" >&2
  exit 1
}
...
flask metrics sync-registry || {
  echo "FATAL: flask metrics sync-registry failed" >&2
  exit 1
}
