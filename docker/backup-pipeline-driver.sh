...
leg_db_backup_and_restore() {
    ...
    export DB_BACKUP_FILE="${DB_BACKUP_DIR}${POSTGRES_DB}_$(date +%Y%m%d_%H%M%S)_daily.sql"
    ...
    local backup_rc
    ( source /app/backup-database.sh ); backup_rc=$?
...
leg_log_backup() {
    ...
    mkdir -p "$LOG_DIR"
    export LOG_FILE="${LOG_DIR}/$(date -d yesterday +%Y-%m-%d)_daily.log"
    export COMPRESSED_LOG_FILE="${LOG_FILE}.gz"
    printf 'sample log line\n' > "$LOG_FILE"

    local log_rc
    ( source /app/backup-logs.sh ); log_rc=$?
...
leg_rclone_default() {
    ...
    (
        ...
        : > /tmp/rclone-calls
        : > "$COMPRESSED_DB_BACKUP_FILE"; : > "$COMPRESSED_LOG_FILE"
        ...
        [ "$remote_rc" -eq 0 ] || { echo "❌ Leg 4a: remote_backup exited $remote_rc"; exit 1; }
        grep -q "copy .* remote:u4i-backups/" /tmp/rclone-calls || { echo "❌ Leg 4a: no database copy to u4i-backups"; exit 1; }
        grep -q "copy .* remote:u4i-logs/" /tmp/rclone-calls || { echo "❌ Leg 4a: no log copy to u4i-logs"; exit 1; }
    ) || exit 1
...
leg_rclone_monthly() {
    ...
    (
        rclone() { echo "rclone $*" >> /tmp/rclone-calls-monthly; return 0; }
        file() { echo "application/gzip"; }
        ...
        [ "$remote_rc" -eq 0 ] || { echo "❌ Leg 4b: remote_backup exited $remote_rc"; exit 1; }
        backup_copies=$(grep -c "remote:u4i-backups/" /tmp/rclone-calls-monthly)
        [ "$backup_copies" -ge 2 ] || { echo "❌ Leg 4b: expected >=2 backup copies (daily+monthly), got $backup_copies"; exit 1; }
        monthly_file="${COMPRESSED_DB_BACKUP_FILE/daily/monthly}"
        [ ! -f "$monthly_file" ] || { echo "❌ Leg 4b: monthly temp file was not cleaned up"; exit 1; }
    ) || exit 1
