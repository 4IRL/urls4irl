...
def upgrade():
    ...
    op.execute(
        """
        DO $$ BEGIN
            CREATE TYPE user_role AS ENUM ('ADMIN', 'USER', 'MOD');
        EXCEPTION
    ...
    op.execute(
        """
        DO $$ BEGIN
            CREATE TYPE member_role AS ENUM ('MEMBER', 'CREATOR', 'CO_CREATOR');
        EXCEPTION
            WHEN duplicate_object THEN null;
        END $$;
    """
    ...
    op.create_table(
        ...
        sa.Column(
            "role",
            postgresql.ENUM(
                "ADMIN", "USER", "MOD", name="user_role", create_type=False
            ),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "memberRole",
            postgresql.ENUM(
                "MEMBER", "CREATOR", "CO_CREATOR", name="member_role", create_type=False
            ),
            nullable=False,
        ),
        sa.ForeignKeyConstraint(
