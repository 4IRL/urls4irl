...
describe("login-form OAuth-only account (errorCode=3)", () => {
  beforeEach(() => {
    document.body.innerHTML = LOGIN_MODAL_HTML;
    vi.clearAllMocks();
  });
  ...
  it("hides #ForgotPasswordLink and surfaces field errors on errorCode=3", async () => {
    const { handleImproperFormErrors } = await import("../init.js");
    const mockDeferred = createMockJqXHR();
    vi.spyOn($, "ajax").mockReturnValue(mockDeferred);
    ...
    // Before-state: the link is visible.
    expect($modal.find("#ForgotPasswordLink").css("display")).not.toBe("none");

    const errorJson = {
      errorCode: 3,
      message: "OAuth only",
      errors: { password: ["This account uses a third-party sign-in."] },
    };
    mockDeferred.reject(
      { status: 400, responseJSON: errorJson, getResponseHeader: vi.fn() },
      "error",
      "Bad Request",
    );
  ...
  it("does NOT hide #ForgotPasswordLink on errorCode=2 (existing behavior)", () => {
    const mockDeferred = createMockJqXHR();
    vi.spyOn($, "ajax").mockReturnValue(mockDeferred);
