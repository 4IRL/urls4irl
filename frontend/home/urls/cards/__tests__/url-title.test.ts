...
describe("URL title edit buttons - tooltip attributes and hover-tooltip hide", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(isCoarsePointer).mockReturnValue(false);
    vi.mocked(isURLTitleSubmitInFlight).mockReturnValue(false);
  });

  it("renders the submit/cancel buttons with the hover tooltip block and aria-labels", () => {
    const { urlCard } = mountTitleBlock();
    const submitBtn = urlCard.find(".urlTitleSubmitBtnUpdate");
    const cancelBtn = urlCard.find(".urlTitleCancelBtnUpdate");
    ...
    expect(submitBtn.attr("data-bs-title")).toBe(
      APP_CONFIG.strings.CONFIRM_URL_TITLE_EDIT_TOOLTIP,
    );
    ...
    expect(cancelBtn.attr("data-bs-title")).toBe(
      APP_CONFIG.strings.CANCEL_URL_TITLE_EDIT_TOOLTIP,
    );
  ...
  it("hides the tooltip even when an in-flight submit blocks the click", () => {
    vi.mocked(isURLTitleSubmitInFlight).mockReturnValue(true);
    const { urlCard } = mountTitleBlock();

    urlCard.find(".urlTitleSubmitBtnUpdate").trigger("click.updateUrlTitle");

    expect(tooltipInstance.hide).toHaveBeenCalled();
    expect(vi.mocked(updateURLTitle)).not.toHaveBeenCalled();
  });
  ...
  it("hides the tooltip when the cancel button is clicked", () => {
    ...
    cancelBtn.trigger("click.updateUrlTitle");
  ...
  it("does not throw on submit or cancel when no tooltip instance exists", () => {
    ...
    expect(() =>
      urlCard.find(".urlTitleCancelBtnUpdate").trigger("click.updateUrlTitle"),
    ).not.toThrow();
