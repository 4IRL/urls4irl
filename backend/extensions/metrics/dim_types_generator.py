...
def generate_resources_ts() -> str:
    """Render the TS source for `RESOURCES`, `ResourceName`, and `RESOURCES_BY_CATEGORY`.

    Walks the `Resource` StrEnum and the `RESOURCE_BY_CATEGORY` dict directly,
    so codegen coverage is guaranteed by the Python source of truth. The
    frontend imports `RESOURCES_BY_CATEGORY[<category>]` at boot to populate
    the per-tab dropdown options.
    """
    parts: list[str] = [_RESOURCES_FILE_HEADER]
    parts.append("")
    parts.append("export const RESOURCES = {")
    for resource in Resource:
        parts.append(f'  {resource.name}: "{resource.value}",')
    parts.append("} as const;")
    parts.append("")
    parts.append(
    ...
    parts.append("")
    parts.append("export const RESOURCES_BY_CATEGORY = {")
    for category in EventCategory:
        resources = RESOURCE_BY_CATEGORY[category]
        values = ", ".join(f'"{resource.value}"' for resource in resources)
        parts.append(f"  {category.value}: [{values}] as const,")
    parts.append("} as const;")
    return "\n".join(parts) + "\n"


def generate_flows_ts() -> str:
    """Render the TS source for `FLOW_IDS`, `FlowId`, and `FLOW_METADATA`.

    Walks the `FlowId` StrEnum and the `FLOWS` registry directly, so codegen
    coverage is guaranteed by the Python source of truth. For each flow,
    `FLOW_METADATA` carries the `displayName` plus a variable-length
    `stepLabels` array built by collecting `step.label` from every `FlowStep`
    in `FlowDefinition.steps`, in funnel order. The funnel-card renderer pairs
    these labels with the per-step counts from the `/api/metrics/query/flow`
    response; nothing here is inferred from the data. The `FlowId` type is
    derived from the `FLOW_IDS` const so the const is the single source of
    truth for both the runtime values and the compile-time type.
    """
    parts: list[str] = [_FLOWS_FILE_HEADER]
    parts.append("")
    parts.append("export const FLOW_IDS = {")
    for flow_id in FlowId:
        parts.append(f'  {flow_id.name}: "{flow_id.value}",')
    parts.append("} as const;")
    parts.append("")
    parts.append("export type FlowId = (typeof FLOW_IDS)[keyof typeof FLOW_IDS];")
    parts.append("")
    parts.append("export const FLOW_METADATA = {")
    for flow_id in FlowId:
        definition = FLOWS[flow_id]
        step_labels = ", ".join(f'"{step.label}"' for step in definition.steps)
        parts.append(f'  "{flow_id.value}": {{')
        parts.append(f'    displayName: "{definition.display_name}",')
        parts.append(f"    stepLabels: [{step_labels}] as const,")
        parts.append("  },")
    parts.append("} as const satisfies Record<FlowId, {")
    parts.append("  displayName: string;")
    parts.append("  stepLabels: readonly string[];")
    parts.append("}>;")
    return "\n".join(parts) + "\n"
