...
UI_OUTPUT="tmp/UI_${TIMESTAMP}_output.txt"
UI_FAILURES="tmp/UI_${TIMESTAMP}_failures.txt"
INT_OUTPUT="tmp/INTEGRATION_${TIMESTAMP}_output.txt"
INT_FAILURES="tmp/INTEGRATION_${TIMESTAMP}_failures.txt"
```

### 2. Run UI Tests (first)

```bash
make test-ui-parallel-built 2>&1 | tee "$UI_OUTPUT"
```
...
```bash
docker exec u4i-local-web /bin/bash -c "source /code/venv/bin/activate && pytest -m 'MARKER'" 2>&1 | tee -a "$UI_OUTPUT"
```
...
```bash
make test-integration-parallel 2>&1 | tee "$INT_OUTPUT"
```
...
```bash
docker compose --project-directory . -f docker/compose.local.yaml exec web bash -c "source /code/venv/bin/activate && python -m pytest -m 'MARKER'" 2>&1 | tee -a "$INT_OUTPUT"
```
