...
description: Run ALL UI tests for URLS4IRL in parallel against the built assets. Use when asked to run all UI tests, run the full UI test suite, or verify all functional/Selenium tests pass. Records failures to tmp/ with timestamped files; cleans up on full success.
---
...
OUTPUT_FILE="tmp/UI_${TIMESTAMP}_output.txt"
FAILURE_FILE="tmp/UI_${TIMESTAMP}_failures.txt"
```
...
```bash
make test-ui-parallel-built 2>&1 | tee "$OUTPUT_FILE"
```
...
```bash
docker exec u4i-local-web /bin/bash -c "source /code/venv/bin/activate && pytest -m 'MARKER'" 2>&1 | tee -a "$OUTPUT_FILE"
```
