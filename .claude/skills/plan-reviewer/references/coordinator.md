...
- **Grounding-Rule enforcement gate (required) — the single check that replaces per-area "read the file" prose:**
  - **Missing-evidence reclassification:** Any finding that references a named code entity (symbol, function, helper, fixture, constant, union member, route, import, file path, config field, established pattern — asserting it exists / is absent / preserved / dropped / differs) with an empty or absent `evidence` field is reclassified `fix_type: "design_decision"`, severity capped at `minor`, with `description` note: "Unverified entity claim — read [file path] and confirm before applying." A `mechanical` fix may never carry an entity claim lacking an `evidence` cite.
  - **Contradiction → conflict:** If two reviewers make incompatible assertions about whether the same named entity exists or is absent — even if both marked `mechanical`, even if both cite evidence — classify the group as `conflict` with `fix_type: "design_decision"` and note: "Resolution requires reading [file path] — do not apply either finding mechanically until the file is read."
