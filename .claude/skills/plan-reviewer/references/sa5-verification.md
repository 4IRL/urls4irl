...
- **Grounding Rule — named test helper/fixture (required):** Apply the Grounding Rule to any test helper, fixture, or utility the plan names (e.g., `wait_then_get_at_least_n_elements`) — grep the specified file before writing any finding or conditional fallback that depends on its presence or absence.
