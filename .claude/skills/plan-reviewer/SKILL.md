...
**Import name-collision guard (required for every DD that adds a new import into an existing shared file):** When a DD's chosen option instructs adding `from X import S` (or any import of bare name S) into an existing shared file (conftest.py, a shared utils module, a shared init file), the DD-application subagent MUST read that file's full existing import block before writing any plan step text and grep for existing bindings of the bare name S. If S is already imported from a different module, the chosen option MUST use an alias (`from X import S as S_alias`) and ALL call sites of the newly added symbol must reference the alias. Never write plan text that adds a bare-name import without completing this collision check — Python silently rebinds the name at module scope and the shadowed call sites fail at runtime with a wrong-signature TypeError.
