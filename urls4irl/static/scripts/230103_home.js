...
function buildTagDeck(dictTags) {
  if (dictTags.length == 0) {
    ...
    $("#listTags")[0].innerHTML =
      "<h5>No Tags Applied to any URLs in this UTub</h5>"; // I still want this to show if user creates a new tag but has not yet applied them to any URLs
  } else {
    // Instantiate UTubDeck (top left panel) with UTubs accessible to current user
...
function buildURLDeck(dictURLs, dictTags) {
  ...
  for (let i in dictURLs) {
    ...
    $(addTag).attr({
      ...
      onclick:
        "cardEdit(" + selectedUTubID + "," + dictURLs[i].url_id + ",'tag')",
    });
    ...
    $(editURL).attr({
      ...
      type: "button",
      onclick:
        "cardEdit(" + selectedUTubID + "," + dictURLs[i].url_id + ",'url')",
    });
    ...
    if (e.type === "blur" || keycode == "13") {
      ...
      request.done(function (response, textStatus, xhr) {
        if (xhr.status == 200) {
          ...
          console.log(
            "Didn't work yet but I'll let it slide for now (needs POST route)",
          );
          URLStringField[0].innerHTML = updatedURLText;
        }
      });
...
function removeTag(tagID) {
  ...
  request.done(function (response, textStatus, xhr) {
    if (xhr.status == 200) {
      ...
      console.log(
        $("div.url[urlid=" + URLID + "]").find("span.tag[" + tagID + "]"),
      );
      $("div.url[urlid=" + URLID + "]")
