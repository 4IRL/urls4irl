...
$(document).ready(function () {
  ...
  $("select").change(function () {
    ...
    $("#removeUserTemp").attr(
      "action",
      "/remove_user/" + selectedUTubID + "/" + $(this)[0].value,
    );
  });
...
function buildUTubDeck(UTubs) {
  ...
  for (i in UTubs) {
    radioHTML +=
      '<label for="UTub-' +
      UTubs[i].id +
      '" class="UTub draw"><input type="radio" id="UTub-' +
      UTubs[i].id +
      '" name="UTubSelection" value="' +
      UTubs[i].name +
      '"><b>' +
      UTubs[i].name +
      "</b></label>";
...
function getUtubInfo(selectedUTubID) {
  ...
  // Pull data from db
  return $.getJSON("/home?UTubID=" + selectedUTubID, function (UTubJSON) {
    buildUTub(UTubJSON);
  });
}
...
function buildURLDeck(dictURLs, dictTags) {
  ...
  for (let i in dictURLs) {
    ...
    urlDescription.innerHTML = dictURLs[i].url_description
...
function tagBadgeBuilder(utubID, urlID, tagDetails) {
  const tagElem = $("<span></span>").addClass(
    "badge badge-pill badge-light tag-badge",
  );
  const tagID = tagDetails.id;
  const tag = tagDetails.tag_string;
  const tagNameElem = $("<span></span>").text(tag);
  const closeButtonOuter = $("<span></span>")
    .prop("aria-hidden", false)
    .attr("id", "tag" + tagID);
  const closeButtonInner = $("<a></a>")
    .addClass("btn btn-sm btn-outline-link border-0 tag-del")
    .html("&times;")
    .prop("href", "#");
...
function buildTagDeck(dictTags) {
  let html =
    '<label for="selectAll"><input id="selectAll" type="checkbox" name="selectAll" checked="true"> Select All </label>';
  ...
  if (dictTags) {
    ...
    for (let i in dictTags) {
      ...
      html +=
        ...
        '" tagid=' +
        tagID +
        '><input class="tagCheckbox" type="checkbox" name="' +
        tagText +
        '" checked="true"> ' +
        tagText +
        " </label>";
...
function createUTub(id, name) {
  radioHTML =
    '<label for="UTub-' +
    id +
    '" class="UTub draw active"><input type="radio" id="UTub-' +
    id +
    '" name="UTubSelection" value="' +
    name +
    '"><b>' +
