...
def login():
    ...
    if not User.query.filter().all():
        ...
        password = generate_password_hash(
            "abcdefg", method="pbkdf2:sha512", salt_length=16
        )
        password2 = generate_password_hash(
            "rehreh", method="pbkdf2:sha512", salt_length=16
        )
        password3 = generate_password_hash(
            "bobob", method="pbkdf2:sha512", salt_length=16
        )
        new_user = User(
            username="Giovanni",
            email="gio@g.com",
...
def register_user():
    ...
    if register_form.validate_on_submit():
        ...
        password = generate_password_hash(
            register_form.password.data, method="pbkdf2:sha512", salt_length=16
        )
        new_user = User(
            username=username, email=email, email_confirm=False, password=password
        )
        db.session.add(new_user)
        db.session.commit()
        user = User.query.filter_by(username=username).first()
...
def delete_utub(utub_id: int):
    ...
    if request.method == "GET":
        return render_template(
            "_delete_utub_form.html", utub_delete_form=utub_delete_form
        )
        ...
        try:
            ...
            return (
                jsonify({"Error": "You don't have permission to delete this UTub!"}),
                404,
            )
...
def update_utub_desc(utub_id: int):
    ...
    if int(current_user.get_id()) not in [
        int(member.user_id) for member in current_utub.members
    ]:
    ...
    utub_description_for_get = {"utub_description": current_utub_description}
    return render_template(
        "add_desc_to_utub.html",
        desc_form=utub_desc_form,
        current_utub_desc=utub_description_for_get,
    )
...
def delete_user(utub_id: int, user_id: int):
    ...
    if int(current_user.get_id()) == int(current_utub.created_by.id):
        ...
        user_to_delete_in_utub = [
            member_to_delete
            for member_to_delete in current_utub.members
            if int(user_id) == (member_to_delete.user_id)
        ][0]

    elif int(current_user.get_id()) in current_user_ids_in_utub and int(user_id) == int(
        current_user.get_id()
    ):
        ...
        flash(
            "Error: Only the creator of a UTub can delete other users. Only you can remove yourself.",
            category="danger",
        )
        return home, 403
...
def add_user(utub_id: int):
    ...
    return render_template(
        "add_user_to_utub.html", utub_new_user_form=utub_new_user_form
    )
...
def delete_url(utub_id: int, url_id: int):
    ...
    url_added_by = [
        url_in_utub.user_that_added_url.id
        for url_in_utub in utub.utub_urls
        if int(url_in_utub.url_id) == int(url_id)
    ]
    ...
    if (
        int(current_user.get_id()) == owner_id
        or int(current_user.get_id()) == url_added_by
    ):
        # User is creator of this UTub, or added the URL
        utub_url_user_row = Utub_Urls.query.filter_by(
...
def add_url(utub_id: int):
    ...
    if utub_new_url_form.validate_on_submit():
        ...
        try:
            ...
            return (
                render_template(
                    "_add_url_form.html", utub_new_url_form=utub_new_url_form
                ),
                400,
            )
            ...
            if already_created_url:
                ...
                urls_in_utub = [
                    utub_user_url_object.url_in_utub
                    for utub_user_url_object in utub.utub_urls
                ]
                ...
                new_url = URLS(
                    url_string=validated_url, created_by=int(current_user.get_id())
                )
...
def add_tag(utub_id: int, url_id: int):
    ...
    utub = Utub.query.get(utub_id)
    utub_url = [
        url_in_utub for url_in_utub in utub.utub_urls if url_in_utub.url_id == url_id
    ]
    user_in_utub = [
        int(member.user_id)
    ...
    if url_tag_form.validate_on_submit():
        ...
        if tag_already_created:
            ...
            if this_tag_is_already_on_this_url:
                ...
                return (
                    render_template("add_tag_to_url.html", url_tag_form=url_tag_form),
                    400,
                )
        ...
        flash(
            f"Added {tag_to_add} to {utub_url[0].url_in_utub.url_string}",
            category="info",
        )
