...
| **Dimensions**     | All UI-category events include auto-injected device_type as an int from the shared `DeviceType` enum (`MOBILE=1`, `DESKTOP=2`) in backend/metrics/events.py, matchMedia-detected at TABLET_WIDTH=992 breakpoint and shipped to the browser via APP_CONFIG.constants.DEVICE_TYPE. Per-event additional dimensions are defined in backend/metrics/dimension_models.py::DIMENSION_MODELS. |
...
| **Query Params** | Required `event_name=<EventName>`; `window=day\|week\|month\|year\|Nh\|Nd` (relative; `month`/`year` calendar-aware) OR `start=<iso>&end=<iso>` (absolute; mutually exclusive with `window`); optional `resolution=hour\|day` (default `hour`)                                                                                                                                                                                                                                                                            |
