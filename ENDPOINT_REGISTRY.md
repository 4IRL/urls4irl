...
| **Service**      | `backend/metrics/query_service.py:latency_percentiles` (exact `percentile_cont` p50/p95/p99 per (endpoint, method), ordered by p95 desc)                                                                                                                                                                                            |
...
| **Query Params** | `window=day\|week\|month\|year\|Nh\|Nd` (relative) OR `start=<iso>&end=<iso>` (absolute; mutually exclusive). Optional `metric_name` (defaults `api_request_duration`), `endpoint`, `method`, `device_type` (1=mobile, 2=desktop), `limit` (1–200, default 25).                                                                       |
| **Response**     | `backend/schemas/metrics.py:LatencyPercentilesResponseSchema` (`rows: list[LatencyPercentileRow]`; each row `endpoint`/`method`/`p50`/`p95`/`p99`/`sample_count`)                                                                                                                                                                    |
...
| **Tests**        | `tests/integration/system/test_metrics_query_endpoints.py` (marker: `cli`), `tests/integration/system/test_metrics_query_service.py` (marker: `cli`), `tests/unit/test_metrics_query_schemas.py` (marker: `unit`); JS units `frontend/admin/__tests__/metrics-query-client.test.ts`, `latency-card.test.ts`, `metrics-dashboard.tablist.test.ts`; Selenium `tests/functional/metrics_ui/test_metrics_dashboard_ui.py` (marker: `metrics_ui`) |
...
| **Service**      | `backend/metrics/query_service.py:latency_timeseries` (zero-filled per-bucket p50/p95/p99 via `date_trunc` + `_truncate_to_resolution`/`_RESOLUTION_STEP`; empty buckets carry `None` percentiles → chart gaps)                                                                                                                      |
...
| **Query Params** | `window=...` OR `start`/`end`; required `endpoint`; optional `metric_name` (defaults `api_request_duration`), `method`, `resolution` (hour default \| day), `device_type` (1=mobile, 2=desktop).                                                                                                                                     |
| **Response**     | `backend/schemas/metrics.py:LatencyTimeseriesResponseSchema` (`buckets: list[LatencyTimeseriesBucket]`; each bucket `bucket`/`p50`/`p95`/`p99`/`sample_count`)                                                                                                                                                                       |
...
| **Tests**        | `tests/integration/system/test_metrics_query_endpoints.py` (marker: `cli`), `tests/integration/system/test_metrics_query_service.py` (marker: `cli`), `tests/unit/test_metrics_query_schemas.py` (marker: `unit`); JS units `frontend/admin/__tests__/metrics-query-client.test.ts`, `render-latency-chart.test.ts`, `latency-card.test.ts`, `metrics-dashboard.tablist.test.ts`; Selenium `tests/functional/metrics_ui/test_metrics_dashboard_ui.py` (marker: `metrics_ui`) |
