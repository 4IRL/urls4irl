...
| **JS Module**      | `frontend/lib/metrics-client.ts` (batches UI events, POSTs to `/api/metrics`. Consumes the generated `MetricsIngestRequest` / `MetricsIngestEvent` types from `frontend/types/api.d.ts`.) Emit call sites span ~30 modules across `frontend/home/` (utubs, urls, tags, members, navbar, collapsible-decks, btns-forms) and `frontend/splash/` (login/register/forgot-password forms, init modal opener/switcher), plus `frontend/lib/csrf.ts` for the global `UI_RATE_LIMIT_HIT` 429 prefilter. See `plans/anonymous-metrics-ui-hooks/anonymous-metrics-ui-hooks.md` for the full inventory.                                                                                                                                                                                                   |
