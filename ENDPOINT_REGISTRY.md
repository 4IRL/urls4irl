...
| **Query Params**   | Optional `transport=beacon` — set by the metrics-client's `flushBeacon()` unload path; `flush()` omits the param. Validated via `TransportQuerySchema`. Phase 13's `API_METRICS_INGEST_BATCH` will consume.                                                                                                                                                                                  |
