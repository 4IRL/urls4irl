...
| **JS Module**      | `frontend/lib/metrics-client.ts` (batches UI events, POSTs to `/api/metrics`; uses `X-CSRFToken` header for `fetch` flushes and JSON-body `csrf_token` fallback for `navigator.sendBeacon`). Consumes the generated `MetricsIngestRequest` / `MetricsIngestEvent` types from `frontend/types/api.d.ts`.                                                                                       |
