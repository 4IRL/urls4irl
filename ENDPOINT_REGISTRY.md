...
| **Request**        | `backend/schemas/requests/metrics.py:MetricsIngestRequest` (top-level batch + optional `batch_id`; `csrf_token` field is deprecated and ignored as of this release (see Step 2)); per-event shape: `MetricsIngestEvent`                                                                                                                                                                     |
...
| **CSRF**           | None — exempt by design. Endpoint is anonymous (no session state to protect), rate-limited per IP, JSON Content-Type forces CORS preflight (handles cross-origin browser abuse), and uses server-side batch_id nonce dedup (handles replay). See plans/anonymous-metrics-csrf-removal/.                                                                                                     |
