...
| **Service**      | `backend/metrics/query_service.py:grouped_counts` (fanned out per step + per drop_breakdown); `backend/metrics/flows.py:FLOWS` registry + `FlowDefinition`/`FlowStep`/`FlowStepBreakdown`; `backend/metrics/flow_ids.py:FlowId`                                                                                                                                                         |
| **Request**      | `backend/schemas/requests/metrics.py:FlowQuerySchema` (+ `FlowIdLiteral`); per-step slicing reuses `FlowFilterCondition` / `FlowFilterParamsSchema`                                                                                                                                                                                                                                    |
