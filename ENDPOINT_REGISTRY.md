...
| **Query Params** | Required `q` (1–100 chars, stripped) — case-insensitive term matched against URL strings, per-UTub titles, and tag text. Validated at runtime via the shared `parse_query_args` (`backend/api_common/parse_request.py`; the `query_schema` decorator arg is OpenAPI metadata only).                                                                                                       |
