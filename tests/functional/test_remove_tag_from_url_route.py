...
def test_remove_tag_from_url_as_utub_creator(add_all_urls_and_users_to_each_utub_with_all_tags, login_first_user_without_register):
    """
    ...
    with app.app_context():
        ...
        utub_this_user_creator_of = Utub.query.filter(Utub.utub_creator == current_user.id).first()
        ...
        tag_url_utub_association = Url_Tags.query.filter(Url_Tags.utub_id == utub_id_this_user_creator_of).first()
        ...
        assert len(Url_Tags.query.filter(Url_Tags.utub_id == utub_id_this_user_creator_of,
                                            Url_Tags.url_id == url_id_to_remove_tag_from,
                                            Url_Tags.tag_id == tag_id_to_remove).all()) == 1

        # Get URL serialization for checking
        initial_url_serialization = Utub_Urls.query.filter(Utub_Urls.utub_id == utub_id_this_user_creator_of,
                                                            Utub_Urls.url_id == url_id_to_remove_tag_from).first().serialized

        # Get all Url-Tag associations count
        initial_url_tag_count = len(Url_Tags.query.all())

        # Get tag count for this UTub and tag
        tag_count = Url_Tags.query.filter_by(utub_id=utub_id_this_user_creator_of, 
                                            tag_id=tag_id_to_remove).count()
    ...
    remove_tag_response = client.post(f"/tag/remove/{utub_id_this_user_creator_of}/{url_id_to_remove_tag_from}/{tag_id_to_remove}", data=add_tag_form)
    ...
    with app.app_context():
        ...
        assert tag_serialized_on_delete_response == Tags.query.get(tag_id_to_remove).serialized

        # Ensure the Tag-URL-UTub association does not exist any longer
        assert len(Url_Tags.query.filter(Url_Tags.utub_id == utub_id_this_user_creator_of,
                                            Url_Tags.url_id == url_id_to_remove_tag_from,
                                            Url_Tags.tag_id == tag_id_to_remove).all()) == 0

        # Grab URL-UTub association
        final_utub_url_association = Utub_Urls.query.filter(Utub_Urls.utub_id == utub_id_this_user_creator_of,
                                                            Utub_Urls.url_id == url_id_to_remove_tag_from).first()
        ...
        assert url_serialized_on_delete_response == final_utub_url_association.serialized

        # Ensure proper number of Url-Tag associations in db
        assert len(Url_Tags.query.all()) == initial_url_tag_count - 1

def test_remove_tag_from_url_as_utub_member(add_all_urls_and_users_to_each_utub_with_all_tags, login_first_user_without_register):
    """
    GIVEN 3 users and 3 UTubs, with all 3 members in each UTub, with 3 URLs in each UTub, and each URL has 3 tags associated with
    ...
    with app.app_context():
        # Get a UTub this user is creator of
        utub_this_user_member_of = Utub.query.filter(Utub.utub_creator != current_user.id).first()
        utub_id_this_user_member_of = utub_this_user_member_of.id
        utub_name_this_user_member_of = utub_this_user_member_of.name

        # Get a URL and tag association within this UTub
        tag_url_utub_association = Url_Tags.query.filter(Url_Tags.utub_id == utub_id_this_user_member_of).first()
        tag_id_to_remove = tag_url_utub_association.tag_id
        url_id_to_remove_tag_from = tag_url_utub_association.url_id

        # Ensure the Tag-URL-UTub association does exists any longer
        assert len(Url_Tags.query.filter(Url_Tags.utub_id == utub_id_this_user_member_of,
                                            Url_Tags.url_id == url_id_to_remove_tag_from,
                                            Url_Tags.tag_id == tag_id_to_remove).all()) == 1

        # Get URL serialization for checking
        initial_url_serialization = Utub_Urls.query.filter(Utub_Urls.utub_id == utub_id_this_user_member_of,
                                                            Utub_Urls.url_id == url_id_to_remove_tag_from).first().serialized
        ...
        # Get tag count for this UTub and tag
        tag_count = Url_Tags.query.filter_by(utub_id=utub_id_this_user_member_of, 
                                            tag_id=tag_id_to_remove).count()
    ...
    remove_tag_response = client.post(f"/tag/remove/{utub_id_this_user_member_of}/{url_id_to_remove_tag_from}/{tag_id_to_remove}", data=add_tag_form)
    ...
    with app.app_context():
        ...
        # Grab URL-UTub association
        final_utub_url_association = Utub_Urls.query.filter(Utub_Urls.utub_id == utub_id_this_user_member_of,
                                                            Utub_Urls.url_id == url_id_to_remove_tag_from).first()
...
def test_remove_tag_from_url_with_one_tag(add_all_urls_and_users_to_each_utub_with_one_tag, login_first_user_without_register):
    """
...
def test_remove_last_tag_from_utub(add_one_url_to_each_utub_one_tag, login_first_user_without_register):
    """
    GIVEN 3 users and 3 UTubs, with all 3 members in each UTub, with 3 URLs in each UTub, and each URL has 1 tag associated
...
def test_remove_tag_from_url_with_five_tags(add_five_tags_to_db_from_same_user, login_first_user_without_register):
    """
    ...
    with app.app_context():
        ...
        tag_to_remove = Url_Tags.query.filter(Url_Tags.utub_id == utub_id_user_is_member_of,
                                                    Url_Tags.url_id == url_id_in_this_utub).first()
        tag_id_to_remove = tag_to_remove.tag_id

        # Get URL serialization for checking
        initial_url_serialization = Utub_Urls.query.filter(Utub_Urls.utub_id == utub_id_user_is_member_of,
                                                            Utub_Urls.url_id == url_id_in_this_utub).first().serialized

        # Get all Url-Tag associations count
        initial_url_tag_count = len(Url_Tags.query.all())

        # Get tag count for this UTub and tag
        tag_count = Url_Tags.query.filter_by(utub_id=utub_id_user_is_member_of, 
                                            tag_id=tag_id_to_remove).count()
    ...
    remove_tag_response = client.post(f"/tag/remove/{utub_id_user_is_member_of}/{url_id_in_this_utub}/{tag_id_to_remove}", data=add_tag_form)
    ...
    with app.app_context():
        ...
        assert len(Url_Tags.query.filter(Url_Tags.utub_id == utub_id_user_is_member_of,
                                            Url_Tags.url_id == url_id_in_this_utub).all()) == 4
        # Grab URL-UTub association
        final_utub_url_association = Utub_Urls.query.filter(Utub_Urls.utub_id == utub_id_user_is_member_of,
                                                            Utub_Urls.url_id == url_id_in_this_utub).first()
...
def test_remove_nonexistent_tag_from_url_as_utub_creator(add_all_urls_and_users_to_each_utub_with_all_tags, login_first_user_without_register):
    """
    GIVEN 3 users and 3 UTubs, with all 3 members in each UTub, with 3 URLs in each UTub, and each URL has 3 tags associated with
    ...
    with app.app_context():
        ...
        valid_url_in_utub = Utub_Urls.query.filter(Utub_Urls.utub_id == utub_id_this_user_creator_of).first()
...
def test_remove_nonexistent_tag_from_url_as_utub_member(add_all_urls_and_users_to_each_utub_with_all_tags, login_first_user_without_register):
    """
    GIVEN 3 users and 3 UTubs, with all 3 members in each UTub, with 3 URLs in each UTub, and each URL has 3 tags associated with
    ...
    with app.app_context():
        ...
        # Get a valid URL within this UTub
        valid_url_in_utub = Utub_Urls.query.filter(Utub_Urls.utub_id == utub_id_this_user_member_of).first()
        url_id_to_remove_tag_from = valid_url_in_utub.url_id
...
def test_remove_tag_from_url_but_not_member_of_utub(add_one_url_to_each_utub_no_tags, add_tags_to_database, login_first_user_without_register):
    """
    ...
    with app.app_context():
        ...
        url_utub_association = Utub_Urls.query.filter(Utub_Urls.utub_id == utub_id_not_member_of).first()
        url_id_in_utub = url_utub_association.url_id

        # Ensure tag does not exist on URL in UTub
        assert len(Url_Tags.query.filter(Url_Tags.utub_id == utub_id_not_member_of).all()) == 0
        ...
        assert len(Url_Tags.query.filter(Url_Tags.utub_id == utub_id_not_member_of).all()) == 1

        initial_url_utub_association = Utub_Urls.query.filter(Utub_Urls.utub_id == utub_id_not_member_of,
                                                                Utub_Urls.url_id == url_id_in_utub).first()
    ...
    remove_tag_response = client.post(f"/tag/remove/{utub_id_not_member_of}/{url_id_in_utub}/{tag_id_to_add}", data=add_tag_form)
    ...
    with app.app_context():
        ...
        final_url_utub_association = Utub_Urls.query.filter(Utub_Urls.utub_id == utub_id_not_member_of,
                                                        Utub_Urls.url_id == url_id_in_utub).first()

        assert initial_url_utub_serialization == final_url_utub_association.serialized
        
def test_remove_tag_from_url_from_nonexistent_utub(add_all_urls_and_users_to_each_utub_with_all_tags, login_first_user_without_register):
    """
    GIVEN 3 users and 3 UTubs, with all 1 member in each UTub, with 1 URL in each UTub, and each URL has no tags associated with it initially
    ...
    with app.app_context():
        ...
        assert len(Url_Tags.query.filter(Url_Tags.utub_id == existing_utub_id,
                                            Url_Tags.url_id == url_id_to_remove,
                                            Url_Tags.tag_id == tag_id_to_remove).all()) == 1

        num_of_url_tag_associations = len(Url_Tags.query.filter(Url_Tags.url_id == url_id_to_remove,
                                                                Url_Tags.tag_id == tag_id_to_remove).all())

        # Ensure URL-Tag association does not exist in nonexistent UTub
        assert len(Url_Tags.query.filter(Url_Tags.utub_id == nonexistent_utub_id,
                                            Url_Tags.url_id == url_id_to_remove,
                                            Url_Tags.tag_id == tag_id_to_remove).all()) == 0

        # Grab initial UTub-URL serialization
        initial_utub_url_association = Utub_Urls.query.filter(Utub_Urls.utub_id == existing_utub_id,
                                                                Utub_Urls.url_id == url_id_to_remove).first()
    ...
    remove_tag_response = client.post(f"/tag/remove/{nonexistent_utub_id}/{url_id_to_remove}/{tag_id_to_remove}", data=add_tag_form)
    ...
    with app.app_context():
        ...
        final_utub_url_association = Utub_Urls.query.filter(Utub_Urls.utub_id == existing_utub_id,
                                                                Utub_Urls.url_id == url_id_to_remove).first()
        ...
        assert num_of_url_tag_associations == len(Url_Tags.query.filter(Url_Tags.url_id == url_id_to_remove,
                                                                Url_Tags.tag_id == tag_id_to_remove).all())

        assert len(Url_Tags.query.all()) == initial_num_tag_url_associations                                                                

def test_remove_tag_from_nonexistent_url_utub(add_all_urls_and_users_to_each_utub_with_all_tags, login_first_user_without_register):
    """
    ...
    remove_tag_response = client.post(f"/tag/remove/{existing_utub_id}/{nonexistent_url_id}/{tag_id_to_remove}", data=add_tag_form)
...
def test_remove_tag_with_no_csrf_token(add_all_urls_and_users_to_each_utub_with_all_tags, login_first_user_without_register):
    ...
    remove_tag_response = client.post(f"/tag/remove/{existing_utub_id}/{url_id_to_remove}/{tag_id_to_remove}", data=add_tag_form)
