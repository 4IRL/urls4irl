...
# CREATE
def test_tag_filter_count_on_tag_badge_creation(
...
def test_tag_filter_count_update_on_tag_badge_addition(
    browser: WebDriver, create_test_tags, app: Flask
):
    """
    Tests the tag filter count in the Tag Deck when a tag badge is added to an existing URL.
...
def test_tag_filter_count_on_tag_filter_creation(
    ...
    num_urls_with_new_tag = count_urls_with_tag_applied_by_tag_id(app, tag_id)

    assert num_urls_with_new_tag == 0
