...
def test_tag_filter_count_after_add_fresh_tag_to_url(
    ...
    add_tag_to_url(
        page=page, selected_url_id=url_in_utub.id, tag_string=UTS.TEST_TAG_NAME_1
    )
...
def test_tag_filter_count_after_add_existing_tag_to_url(
    ...
    add_tag_to_url(page=page, selected_url_id=url_id, tag_string=new_tag_string)
    btn_selector = f"{HPL.ROW_SELECTED_URL} {HPL.BUTTON_TAG_SUBMIT_CREATE}"
    wait_then_click_element(page=page, css_selector=btn_selector)
...
def test_tag_filter_count_update_with_multiple_tag_filters_applied(
    ...
    second_vis_init, second_total_init = (
        get_visible_urls_and_urls_with_tag_text_by_tag_id(
            page=page, tag_id=second_tag_id
        )
    )
...
def test_create_fresh_tag_sets_count(page: Page, create_test_urls, provide_app: Flask):
    """
...
def test_create_non_fresh_tag_increments_counts(
    ...
    add_tag_to_url(
        page=page, selected_url_id=url_in_utub.id, tag_string=tag_already_in_utub_str
    )
