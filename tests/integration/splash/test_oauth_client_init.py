...
the exact extensions key Authlib registers). Also confirms the Google and
GitHub clients themselves are registered (`oauth.google`/`oauth.github`) when
credentials are configured, and that `should_register_google_oauth`/
`should_register_github_oauth`'s guard conditions are correct in isolation.
...
`test_github_oauth_client_registered_with_configured_credentials` therefore
assert against the shared session-scoped `app` fixture (which the `build_app`
fixture in `tests/conftest.py` always creates with dummy Google/GitHub
credentials) rather than constructing a second, credential-less `create_app()`
to prove a negative — a second call cannot reliably prove `oauth.google`/
`oauth.github` is absent, since a prior registration in the same interpreter
persists.
"""
...
def test_github_oauth_client_registered_in_app_extensions(app: Flask):
    """
    GIVEN a created app that calls `oauth.init_app(app)` in `create_app`
    WHEN inspecting the app's registered extensions
    THEN Authlib's exact extensions key is present (the same singleton
        `oauth.init_app` call registers both the Google and GitHub clients)
    """
    with app.app_context():
        assert AUTHLIB_EXTENSION_KEY in current_app.extensions


def test_github_oauth_client_registered_with_configured_credentials(app: Flask):
    """
    GIVEN the shared test app, built with dummy GitHub OAuth credentials via
        `tests/conftest.py`'s `build_app` fixture
    WHEN `create_app` runs its guarded `oauth.register(name="github", ...)` call
    THEN `oauth.github` is registered and its `client_id` matches the
        configured value
    """
    with app.app_context():
        assert oauth.github is not None
        assert oauth.github.client_id == TEST_GITHUB_OAUTH_CLIENT_ID
...
def test_should_register_github_oauth(
    client_id: str | None, client_secret: str | None, expected: bool
):
    """
    GIVEN four combinations of GitHub OAuth client_id/client_secret presence
    WHEN calling `should_register_github_oauth` directly (no `create_app()`,
        no Flask app, no Authlib registry involved)
    THEN it returns `True` only when both values are present
    """
    assert should_register_github_oauth(client_id, client_secret) is expected
