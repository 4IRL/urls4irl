...
def _self_deactivate_user(app: Flask, user_id: int) -> None:
    """Put ``user_id`` into the reversible self-deactivated state: suspended
    with a ``self_deactivated_at`` stamp (what ``perform_self_deactivation``
    will set once Step 2's endpoint exists — mirrors ``_suspend_user`` for the
    admin case)."""
    with app.app_context():
        target_user: Users = Users.query.get(user_id)
        target_user.is_suspended = True
...
def _seed_admin_user(app: Flask) -> int:
    """Seed a second, email-validated ADMIN user (acting admin for the
    admin-override scenario) and return its id."""
    with app.app_context():
        admin_user = Users(
            username="acting_admin",
            email="acting_admin@example.com",
            plaintext_password="acting_admin_password",
        )
...
# ---------------------------------------------------------------------------
# Reactivate-on-login (DD-B): a reversible self-deactivation
# (is_suspended=True, self_deactivated_at set) is auto-lifted on the next
# successful credential/subject check; a hard admin suspension
# (self_deactivated_at IS NULL) is never lifted.
# ---------------------------------------------------------------------------


def test_self_deactivated_user_reactivated_on_web_login(
    app: Flask, register_first_user
):
    """
    GIVEN a registered, validated user in the reversible self-deactivated
        state (is_suspended=True, self_deactivated_at set)
    WHEN they POST /login with correct credentials
    THEN they are reactivated (both flags cleared), the login succeeds (200),
        and the authenticated session reaches /home.
    """
    _, registered_user = register_first_user
    registered_user_id: int = registered_user.id
    _self_deactivate_user(app, registered_user_id)
    ...
    with app.app_context():
        reactivated_user: Users = Users.query.get(registered_user_id)
...
def test_wrong_password_on_self_deactivated_account_does_not_reactivate(
    app: Flask, register_first_user
):
    """
    GIVEN a self-deactivated account
    WHEN a login is attempted with the WRONG password
    THEN it is rejected (400) and neither flag is cleared — reactivation
        never runs on a failed credential check.
    """
    _, registered_user = register_first_user
    registered_user_id: int = registered_user.id
    _self_deactivate_user(app, registered_user_id)
    ...
    wrong_password_payload = {
        LOGIN_FORM.USERNAME: valid_user_1[LOGIN_FORM.USERNAME],
        LOGIN_FORM.PASSWORD: "not-the-real-password",
    }
    login_response = client.post(
        login_url, json=wrong_password_payload, headers={"X-CSRFToken": csrf_token}
    )
    assert login_response.status_code == 400

    with app.app_context():
        untouched_user: Users = Users.query.get(registered_user_id)
        assert untouched_user.is_suspended is True
        assert untouched_user.self_deactivated_at is not None


def test_admin_suspended_user_not_reactivated_on_web_login(
    app: Flask, register_first_user
):
    """
    GIVEN an ADMIN-suspended user (is_suspended=True, self_deactivated_at IS
        NULL — a hard lock, not a self-pause)
    WHEN they POST /login with correct credentials
    THEN they still get 403 ACCOUNT_SUSPENDED and the flag stays NULL —
        reactivate-on-login never touches an admin lock.
    """
    _, registered_user = register_first_user
    registered_user_id: int = registered_user.id
    _suspend_user(
        app, registered_user_id
    )  # admin-style: self_deactivated_at stays NULL
    ...
    with app.app_context():
        still_locked_user: Users = Users.query.get(registered_user_id)
        assert still_locked_user.is_suspended is True
        assert still_locked_user.self_deactivated_at is None


def test_admin_suspend_of_self_deactivated_user_clears_flag_and_refuses_login(
    app: Flask, register_first_user
):
    """
    GIVEN a self-deactivated user and a separate acting admin
    WHEN the admin suspends that already self-deactivated user
    THEN self_deactivated_at is cleared (converting the self-pause into a true
        admin lock) and a subsequent correct-credential login is refused (403)
        — the admin override cannot be bypassed by reactivate-on-login.
    """
    _, registered_user = register_first_user
    target_user_id: int = registered_user.id
    _self_deactivate_user(app, target_user_id)
    acting_admin_id: int = _seed_admin_user(app)

    with app.app_context():
        # suspend_user returns a Flask (body, status_code) tuple.
        _suspend_body, suspend_status_code = suspend_user(
            actor_id=acting_admin_id,
            target_user_id=target_user_id,
            reason="admin override of self-pause",
        )
        assert suspend_status_code == 200

        admin_locked_user: Users = Users.query.get(target_user_id)
        assert admin_locked_user.is_suspended is True
        assert admin_locked_user.self_deactivated_at is None
    ...
    assert login_response.status_code == 403
    assert login_response.get_json()[STD_JSON.MESSAGE] == USER_FAILURE.ACCOUNT_SUSPENDED


@mock.patch(_AUTHORIZE_ACCESS_TOKEN_TARGET)
def test_self_deactivated_oauth_user_reactivated_at_google_callback(
    mock_authorize_access_token: mock.MagicMock, app: Flask, load_login_page
):
    """
    GIVEN a self-deactivated user with a linked google UserOAuthIdentity
    WHEN the Google callback resolves that identity (the provider-subject
        proof succeeds)
    THEN the account is reactivated (both flags cleared) and the callback
        completes with an authenticated redirect — mirroring the web-login
        reactivate path for the OAuth entry point.
    """
    mock_authorize_access_token.return_value = _build_mocked_google_token(
        subject=_OAUTH_SUBJECT, email=_OAUTH_EMAIL
    )
    user_id = _seed_linked_google_user(
        app, subject=_OAUTH_SUBJECT, email=_OAUTH_EMAIL, username=_OAUTH_USERNAME
    )
    _self_deactivate_user(app, user_id)
    client, _ = load_login_page
    ...
    with app.app_context():
        reactivated_user: Users = Users.query.get(user_id)
        assert reactivated_user.is_suspended is False
        assert reactivated_user.self_deactivated_at is None
    mock_authorize_access_token.assert_called_once()
